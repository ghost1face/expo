...
On Android and iOS, `ReadableStream` request bodies are also streamed to the network (chunked upload) instead of being buffered in memory first. You can pass `duplex: 'half'` to match the Fetch standard when sending a stream body:
